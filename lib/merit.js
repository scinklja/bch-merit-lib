...
class Merit {
  ...
  async calcMerit (hydratedUtxos, address, tokenId) {
    try {
      ...
      if (process.env.MERIT_AGE === 1) { currentBlockHeight = await this.bchjs.Blockchain.getBlockCount() }
      ...
      for (let i = 0; i < hydratedUtxos.length; i++) {
        ...
        if (process.env.MERIT_AGE === 1) { merit = merit * age }
